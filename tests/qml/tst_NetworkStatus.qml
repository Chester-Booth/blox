...
TestCase {
    ...
    Services.NetworkStatus {
        ...
        providerSource: "file:/nonexistent/blox-network-provider.qml"
        interval: 60000
    }

    function init() {
        status.providerSource = "file:/nonexistent/blox-network-provider.qml";
    }
    ...
    function test_action_uses_the_loaded_provider() {
        status.providerSource = Qt.resolvedUrl("fixtures/FakeNetworkProvider.qml");
        tryCompare(status, "providerReady", true);
        const result = status.action("set-wifi", "on");
        verify(result.ok);
        compare(result.code, "ok");
        compare(result.data.operation, "set-wifi");
