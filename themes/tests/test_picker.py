...
class PickerIntegrationSourceTests(unittest.TestCase):
    ...
    def test_advanced_picker_can_toggle_place_and_reorder_every_bar_item(self) -> None:
        ...
        self.assertIn('availableGpuModes: ["dedicated", "hybrid", "integrated"].filter', advanced)
        self.assertIn('["always"].concat(section.availableGpuModes)', advanced)
        self.assertIn('"hide in " + value.charAt(0).toUpperCase()', advanced)
        self.assertIn('model: gpuItem ? visibilityValues.map', advanced)
        self.assertIn('(gpuItem && section.availableGpuModes.length > 0)', advanced)
